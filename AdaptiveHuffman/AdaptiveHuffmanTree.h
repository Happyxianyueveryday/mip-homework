 ...
 * _find_pos: 查找目标结点
 * param chr: 目标结点的字符
 * note: 若查找到了目标结点，则返回目标结点的指针；若未查找到目标结点，则返回NEW结点的指针；若哈夫曼树为空，则返回空结点 
 */
...
/** 
 * _get_tar: 获取层次遍历中第一个权值为输入结点减1的结点
 */ 
 ...
 * note 1: 注意root1->parent和root2->parent相等的特殊情况 
 * note 2: 该方法不更新交换后各个父结点的权值 
 */
